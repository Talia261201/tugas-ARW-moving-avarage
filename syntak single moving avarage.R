...
hujan <- read_csv("~/SEMESTER 6/Analisis runtun waktu/hujan (1).csv", sep="")
hujan
...
#peramalan 5 periode kedepan pada m optimal
ts_data3<-c(phujan.sma3)
ts_data3<-(ts(phujan.sma3, start=c(1813)))
library(zoo)
imputasi_data<-na.spline(ts_data3)
model_ets<-ets(imputasi_data)
peramalan5period<-forecast(model_ets,h = 5)
print(peramalan5period)
